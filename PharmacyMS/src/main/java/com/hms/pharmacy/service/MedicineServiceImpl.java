...
public class MedicineServiceImpl implements MedicineService{
    ...
    public Long addMedicine(MedicineDTO medicineDTO) throws HmsException {
        Optional<Medicine> optionalMedicine = medicineRepository.findByNameIgnoreCaseAndDosageIgnoreCase(medicineDTO.getName(), medicineDTO.getDosage()).stream().findFirst();
    ...
    public MedicineDTO getMedicineById(Long id) throws HmsException {
        return medicineRepository.findById(id).orElseThrow(() -> new HmsException("MEDICINE_NOT_FOUND")).toMedicineDTO();
    }
    ...
    public void updateMedicine(MedicineDTO medicineDTO) throws HmsException {
        ...
        if (!(medicineDTO.getName().equalsIgnoreCase(existingMedicine.getName()) && medicineDTO.getDosage().equalsIgnoreCase(existingMedicine.getDosage()))) {
            Optional<Medicine> optional = medicineRepository.findByNameIgnoreCaseAndDosageIgnoreCase(medicineDTO.getName(), medicineDTO.getDosage()).stream().findFirst();
    ...
    public Page<MedicineDTO> getAllMedicines(Pageable pageable) throws HmsException {
        return medicineRepository.findAll(pageable).map(Medicine::toMedicineDTO);
    }
    ...
    public long getLowStockCount(Integer threshold) {
        return medicineRepository.countByStockLessThan(threshold);
    }
}
